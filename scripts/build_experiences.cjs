...
if (rawExperiences.length !== 100) {
  console.error(`Error: esperado 100 elementos pero hay ${rawExperiences.length}`);
  process.exit(1);
}

const experiences = rawExperiences.map((exp, index) => ({
  id: String(index + 1),
  ...exp
}));

const tsContent = `// Wonderlust Explorer - Dataset de Experiencias
...
const outputPath = path.join(targetDir, 'experience.ts');
fs.writeFileSync(outputPath, tsContent, 'utf8');
console.log(`Generado exitosamente ${outputPath} con ${experiences.length} experiencias.`);
